...
As the EDA has shown, `Activity_Score` does not seem to have a relationship with `Indifferent`, so we take it out for Model 2. We conduct a drop-in-deviance test that confirms that the coefficient of `Activity_Score` is not statistically significant from 0 because the p-value of the test (0.99) was greater than 0.05.
...
As observed below, Model 2 has lower AIC (2771.92 \< 2773.76) and BIC (2838.60 \< 2846.03) than Model 1. It also has marginally higher accuracy (0.519 \> 0.518). Therefore, Model 2 is the better model and we will use it for the rest of our analysis.
...
The linearity condition is not satisfied. There is not a linear relationship between the empirical logit and the predictor variable of `Above Ground Sighter Measurement`. We could fix this condition if we had more specific data with fewer NA values. Additionally, by converting FALSE values to zeros, we may have caused the variable to not have a linear relationship with the empirical logit. Because the condition is not satisfied, the coefficients and p-value for `Above Ground Sighter Measurement` in our model may not be accurate, and we are unable to conclude if `Above Ground Sighter Measurement` is a significant predictor.
...
Of all the variables we examined, `QuaasTRUE` and `Primary Fur Color_Gray` were the only terms that had coefficients with significant p-values. The odds that a squirrel is indifferent to a human is multiplied by a factor of `r round(exp(-0.956), 3)` ($\exp(-0.956)$) if it quaas, holding all else constant. If a squirrel's fur color is gray, the odds that it its indifferent to humans is multiplied by a factor of `r round(exp(0.48), 3)` ($\exp(-0.480)$), holding all else constant. Although `Above Ground Sighter Measurement` is not significant at the $\alpha = 0.05$ level, it is significant at the $\alpha = 0.10$ level. For every additional 1 meter in the squirrel's location above ground, the odds that it is indifferent is multiplied by a factor of `r round(exp(-0.009), 3)` ($\exp(-0.009)$), holding all else constant. In other words, quaas, and being above ground decrease the odds that a squirrel is indifferent to humans.

The model does not have a high predictive power as shown by the low AUC (0.534) and accuracy `r round((99+198) / (74+197+99+198), 3)`) ($\frac{99+198}{74+197+99+198}$). This is somewhat expected given what we saw in the EDA section, where the distribution of the predictors is either similar across the two levels of response or very imbalanced.

# Discussion and Conclusion

In this paper, we have investigated which factors significantly affect whether a squirrel is indifferent to human presence, using a logistic regression model and a 10-fold cross-validation for model selection. We concluded that based on our logistic model, there are three significant predictors of `Indifferent`, which are `Quaas`, `Primary Fur Color_Gray` and `Above Ground Sighter Measurement` since their p-values in the model is less than the $\alpha = 0.10$ significance level. Putting this into a larger context, this shows that the squirrels' attitude to humans are largely caused by human presence since `Quaas` are sounds squirrels make when they see a ground threat (either a human or perhaps a pet dog), and our model shows that its presence is predicted to decrease the squirrels' odds of being indifferent.
...
Although `Above Ground Sighter Measurement` had a statistically significant coefficient at $\alpha = 0.10$, the actual value of the coefficient is near-zero. The fact that it is negative is difficult to interpret because it suggests that the higher the squirrel is from the ground, the lower the odds of it being indifferent to the researchers. However, logically, we might argue that the higher the squirrel is off the ground, the safer it should feel from the researchers. Or perhaps, there are confounding factors: maybe the squirrels on trees are skittish due to another threat.

We do not have a scientific justification for why `Primary Fur Color_Gray` would be statistically significant, but we suspect this is due to the fact that the vast majority of squirrel observed had gray fur. We did not find any scientific literature that pointed to a difference in behavior between squirrels that had gray fur versus those that had cinnamon or black fur.
