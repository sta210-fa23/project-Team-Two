...
```{r}
squirrels |>
  select(c(Quaas, `Tail flags`)) |>
  mutate(Quaas_d = ifelse(Quaas == TRUE, 1, 0), 
         Tail_flags_d = ifelse(`Tail flags` == TRUE, 1, 0))
```
...
```{r}
squirrels_rec <- recipe(Indifferent ~ Activity_Score + Age + `Primary Fur Color` + Location + `Above Ground Sighter Measurement` + Kuks + Quaas + Moans + `Tail flags` + `Tail twitches`,
                    data = squirrels_train) |>
  step_dummy(all_nominal_predictors())

squirrels_workflow = workflow() |>
  add_model(squirrels_spec) |>
  add_recipe(squirrels_rec)
```
